...
class Database
{
    ...
    public function __construct() 
    {
        if(empty(self::$_mysqli))
        {
            ...
            {
                die('Connect Error: ' . self::$_mysqli->connect_errno);
            }
        }
    }
    ...
    protected function has_many($method, array $relationship){
        $this->{$method} = create_function('$object',
    ...
    private function separateInPages($page_size,$page_var,$prepared,$bindParams) {
        ...
        if($size>0) {
            ...
            if (isset($_REQUEST[$page_var])) $page = $_REQUEST[$page_var]; else $page = 1;
            ...
            //GET
            if(Pagination::$config['use_get'] === true)
            {
                $previous = ($page-1);
                $next = ($page+1);
                
                if($page>1) $links.='<a class="pagination_previous" 
                ...
                for($i=($page-2);$i<=$count&&$i<=$page+3;$i++) {
                    if($i>0) {
                        if($i!=$page) $links.=' <a class="numbers" href="?'.$page_var.'='.$i.$argum.'" '.$title.$i.'">'.$i.'</a> ';
                        else $links.=' <b class="active_page">'.$i.'</b>';
                    }
                }
                if($page<$count) $links.='<a class="pagination_next" href="?'.$page_var.'='.($page+1).$argum.'" '.$title.$next.'">'.Pagination::$config['next'].'</a>  
            ...
            else
            {
                ...
                if($page>1) $links.='<a class="pagination_previous" href="'.$url.'1'.$arguments.'" '.$title.'1">'.Pagination::$config['first'].'</a> 
                ...
                for($i=($page-2);$i<=$count&&$i<=$page+3;$i++) 
                {
                    if($i>0) 
                    {
                        if($i!=$page) $links.=' <a class="numbers" href="'.$url.$i.$arguments.'" '.$title.$i.'">'.$i.'</a> ';
                        else $links.=' <b class="active_page">'.$i.'</b>';
                    }
                }
                if($page<$count) $links.='
    ...
    private function genLimit($page_size, $page_var) 
    {
        ...
        $_REQUEST[$page_var] = (int)$_REQUEST[$page_var];
        if ($_REQUEST[$page_var]) $page = $_REQUEST[$page_var]; else $page = 1;
